...
echo This setup installs MeloMate plus CUDA 12.1 PyTorch for NVIDIA GPU voice cloning.
...
"%ROOT%backend\.venv\Scripts\python.exe" -m pip install --force-reinstall -r "%ROOT%backend\requirements-gpu-cu121.txt"
if errorlevel 1 (
  echo [ERROR] Failed to install CUDA PyTorch packages.
  echo If this computer does not support CUDA 12.1, use setup-windows.bat instead.
  pause
  exit /b 1
)
