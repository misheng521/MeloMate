...
"%ROOT%backend\.venv\Scripts\python.exe" -c "import importlib.util as u; mods=['fastapi','uvicorn','websockets','loguru','pydantic','yaml','numpy','soundfile','httpx','requests','aiohttp','openai','anthropic','edge_tts','pysbd','langdetect','pydub','sherpa_onnx','onnxruntime','multipart','chardet','jinja2','tqdm','mcp','letta_client','torch','torchaudio','transformers','accelerate','librosa','omnivoice']; missing=[m for m in mods if u.find_spec(m) is None]; print('Missing backend dependencies: '+', '.join(missing)) if missing else print('Backend dependencies verified.'); raise SystemExit(1 if missing else 0)"
if errorlevel 1 (
