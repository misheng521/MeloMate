...
class WebSocketHandler:
    ...
    async def _handle_client_api_config(
        ...
        """Apply per-client API settings sent by the mobile frontend."""
        context = self.client_contexts.get(client_uid)
        if not context:
            await websocket.send_text(
                json.dumps(
        ...
        base_url = data.get("api_base_url") or data.get("base_url") or ""
        ...
        try:
            ...
            await websocket.send_text(
                json.dumps(
                    {
                        "type": "full-text",
                        "text": f"API config applied: {model}",
                    }
                )
            )
        except Exception as e:
            logger.error(f"Failed to apply client API config: {e}")
            await websocket.send_text(
                json.dumps(
                    {
                        "type": "error",
                        "message": f"Failed to apply API config: {e}",
                    }
                )
            )
    ...
    async def _handle_client_voice_clone_config(
        ...
        if not context:
            await websocket.send_text(
                json.dumps({"type": "error", "message": "Client context is not initialized"})
            )
            return
        ...
        try:
            if enabled:
                ...
                if not audio_base64:
                    raise ValueError("Voice clone is enabled, but no reference audio was provided")

                if "," in audio_base64:
                    audio_base64 = audio_base64.split(",", 1)[1]

                raw_audio = base64.b64decode(audio_base64)
                suffix = Path(data.get("file_name") or "reference.wav").suffix.lower()
                if suffix not in {".wav", ".mp3", ".flac", ".m4a", ".ogg"}:
                    suffix = ".wav"

                ref_dir = Path("..") / "reference_sounds" / "voice_clone_refs" / client_uid
                ref_dir.mkdir(parents=True, exist_ok=True)
                ref_file = ref_dir / f"reference{suffix}"
                ref_file.write_bytes(raw_audio)
            ...
            await websocket.send_text(
                json.dumps(
                    {
                        "type": "error",
                        "message": f"Failed to apply voice clone config: {e}",
                    }
                )
            )
