...
from .utils import ensure_sense_voice_minimal_model
import onnxruntime
...
class VoiceRecognition(ASRInterface):
    ...
    def _create_recognizer(self):
        if self.model_type == "transducer":
            ...
            if (
                not self.sense_voice
                or not os.path.isfile(self.sense_voice)
                or not self.tokens
                or not os.path.isfile(self.tokens)
            ):
                if self.sense_voice and self.sense_voice.startswith(
                    "../models/backend/sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17"
                ):
                    logger.warning(
                        "SenseVoice model files not found. Downloading minimal runtime files..."
                    )
