...
def ensure_sense_voice_minimal_model(output_dir: str | Path | None = None) -> Path:
    """Download only the two SenseVoice files used by the runtime."""
    model_dir_name = "sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17"
    backend_models_dir = Path(output_dir) if output_dir is not None else project_backend_models_dir()
    ...
    if model_file.exists() and tokens_file.exists():
        logger.info(f"SenseVoice minimal model already exists: {model_dir}")
        return model_dir

    base_url = f"https://huggingface.co/csukuangfj/{model_dir_name}/resolve/main"
    if not model_file.exists():
        download_file(f"{base_url}/model.int8.onnx", model_file)
    if not tokens_file.exists():
        download_file(f"{base_url}/tokens.txt", tokens_file)
