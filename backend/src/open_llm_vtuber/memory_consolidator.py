...
保留理解表达所需的情境与范围，例如“讨论方案时坚持先验证”，不概括成“性格固执、以后都要坚持”。不同情境下的不同表现可以并存，不必合并成固定标签或解释为性格已经改变。
不把人设设定或对它的复述复制成实际经历，不因回答符合设定就强化对应特征。有回查价值的明确设定变更可记录是谁作了什么说明，并标明属于设定；不能写成角色实际做过的事。
...
summary 是滚动的较早对话摘要，接续 previous_summary，保留发生过的事、当前目标与未结束内容；它不是人设或关系指令。摘要同样保留情境与归属，不用固定性格标签替代具体事件，不要求下次怎样回应。
